    ...
    try:
        ...
        if any(objects):
            log.warning(
                ...
                f"[bold yellow]'{filetype}'[/bold yellow]."
            )
            return None
        ...
        log.info(
            f"Created [bold blue]{result.object_name}[/bold blue] object; etag:"
            f"[bold red]{result.etag}[/bold red], version-id: [bold magenta]{result.version_id}[/bold magenta]"
        )
        return True
    except S3Error as e:
