...
def ingest(
    ...
    log.warning(f"Response status: {response['status']}, From cache: {response['from_cache']}")
...
if __name__ == "__main__":
    ...
    if result is True:
        log.warning("[bold green]Ingestion completed successfully.[/bold green]")
    elif result is False:
        log.warning("[bold red]Ingestion failed.[/bold red]")
    else:
